############################################################ #
# Script name: "4_1_compute_apis_metrics.R"
...
# Output: apis_network_metrics.rds (data frame in environment)
#
# Author: L. Marini
# Reviewer: J.B. Lanuza
# Year: 2025
...
#### Load data ####
filtered_interaction_data = readRDS("Data/filtered_interaction_data.rds")

#### Compute total Apis interactions and degree per network ####
apis_interactions = filtered_interaction_data %>%
...
apis_interaction_evenness = filtered_interaction_data %>%
  filter(pollinator == "Apis mellifera") %>%
...
apis_simpson = filtered_interaction_data %>%
  filter(pollinator == "Apis mellifera") %>%
  group_by(id, plant) %>%
...
all_interactions = filtered_interaction_data %>%
  group_by(id) %>%
  summarise(
...
#### Save data ####
saveRDS(apis_summary, "Data/apis_network_metrics.rds")
