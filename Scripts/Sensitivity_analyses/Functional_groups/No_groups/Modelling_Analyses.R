...
#
# Input:
#   - Data/df_mod.rds     (final modeling dataset)
#
# Output:
#   
# Author: L. Marini
...
#### Model 1:  SES(PDI) ####
fit_SES_pdi = glmmTMB(
  SES_pdi ~  scale(Dominance) + Habitat + scale(Temperature) + scale(log(Plant))*scale(Dominance)+scale(I(Dominance^2))+
...
fit_SES_nd <- glmmTMB(
  SES_norm_degree ~  scale(Dominance) + Habitat + scale(Temperature) + scale(log(Plant))*scale(Dominance)+scale(I(Dominance^2))+
    (1|study_id/network_id/date),
  dispformula = ~scale(Dominance)+scale(log(Plant))+
    (1|study_id/network_id/date),
...
names(modelling_data)
fit_SES_mh <- glmmTMB(
  SES_morisita_horn ~  scale(Dominance)+scale(Plant) + Habitat + scale(Temperature) + scale(Plant)+scale(Dominance)+
    (1|study_id/network_id/date),
...
#### Save data ####
saveRDS(fit_SES_pdi, "Data/fit_SES_pdi_nogroup.rds")
saveRDS(fit_SES_nd, "Data/fit_SES_nd_nogroup.rds")
saveRDS(fit_SES_mh, "Data/fit_SES_mh_nogroup.rds")
